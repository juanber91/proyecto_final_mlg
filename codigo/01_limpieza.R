...
narco <- read.csv('Final/proyecto_final_mlg/datos/completa/EJECUCIONES (2006-2011).csv',
                  sep = ';', encoding = 'latin1') %>% 
...
tabla1 <- readxl::read_excel('Final/proyecto_final_mlg/datos/completa/tabla1-EJECUCIONES.xlsx') %>% 
...
tabla2 <- readxl::read_excel('Final/proyecto_final_mlg/datos/completa/tabla1-EJECUCIONES.xlsx') %>% 
  setNames(c('AÑO_ID', 'DRUGS')) %>% 
  mutate(DRUGS = ifelse(DRUGS %in% c(1,3), DRUGS, 2)) %>%
  count(AÑO_ID, DRUGS) %>% 
  mutate(n = 1) %>% 
  spread(DRUGS, n, 0) %>% 
  setNames(c('AÑO_ID', 'd_cocaina', 'd_otras_drogas', 'd_extasis'))

tabla3 <- readxl::read_excel('Final/proyecto_final_mlg/datos/completa/tabla3-EJECUCIONES.xlsx') %>% 
...
tabla4 <- readxl::read_excel('Final/proyecto_final_mlg/datos/completa/tabla4-EJECUCIONES.xlsx')  %>% 
  setNames(c('AÑO_ID', 'WEAPON')) %>% 
  count(AÑO_ID) %>% 
  mutate(n = 1) %>% 
  rename(WEAPON = n)

# tabla5 <- readxl::read_excel('Final/proyecto_final_mlg/datos/2010/table5-A-X10.xlsx') %>% 
#   setNames(c('AÑO_ID', 'CALIBER'))

tabla6 <- readxl::read_excel('Final/proyecto_final_mlg/datos/completa/tabla6-EJECUCIONES.xlsx') %>% 
...
tabla7 <- readxl::read_excel('Final/proyecto_final_mlg/datos/completa/tabla7-EJECUCIONES.xlsx') %>% 
...
tabla8 <- readxl::read_excel('Final/proyecto_final_mlg/datos/completa/tabla8-EJECUCIONES.xlsx') %>% 
...
write.csv(narco_full, 'Final/proyecto_final_mlg/datos/limpios.csv')
